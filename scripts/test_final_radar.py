"""
Test final: radar con flip_x activado y posicionado abajo-centro
"""
import sys
...
def create_test_frame(video_path, frame_num, output_path):
    """Crea un frame de prueba con el radar corregido"""
    ...
    if pitch_results.keypoints is not None and len(pitch_results.keypoints) > 0:
        ...
        if len(valid_keypoints) >= 4:
            ...
            # Transformar posiciones de jugadores CON FLIP_X
            if len(person_detections) > 0:
                ...
                # CON flip_x=True para corregir inversión
                transformed_positions = transformer.transform_points(player_positions, flip_x=True)

                # Crear radar
                points_dict = {'team1': transformed_positions}
                radar_view = draw_radar_view(pitch_config, points_dict, scale=8)

                # Horizontal (sin rotación), 30% del ancho
                scale_factor = 0.30
                ...
                margin_bottom = 20
                offset_x = (width - new_w) // 2  # Centrado
                offset_y = height - new_h - margin_bottom  # Abajo
    ...
    print(f"Configuracion: flip_x=True, posicion=centro-abajo, transparencia=65%")


if __name__ == "__main__":
    video_path = Path("inputs/2_720p_clip_5-20.mp4")
    output_path = Path("outputs/test_final_radar.jpg")
    output_path.parent.mkdir(exist_ok=True)

    print("Generando frame de prueba con configuracion final...")
    create_test_frame(video_path, 250, output_path)
    print(f"\nFrame generado en: {output_path}")
    print("\nCambios aplicados:")
    print("  1. flip_x=True para corregir inversion horizontal")
    print("  2. Posicion: centro-abajo del video")
    print("  3. Transparencia: 65% para ver la jugada")
